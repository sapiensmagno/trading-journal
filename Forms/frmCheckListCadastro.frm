VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} frmCheckListCadastro 
   Caption         =   "Cadastro de Itens para CheckList"
   ClientHeight    =   7096
   ClientLeft      =   96
   ClientTop       =   414
   ClientWidth     =   11550
   OleObjectBlob   =   "frmCheckListCadastro.frx":0000
   StartUpPosition =   1  'CenterOwner
end
attribute vb_name = "frmchecklistcadastro"
...
const stablename as string = "tbchecklist"
const stablefields as string = "tbchecklistcampos"
const stableid as string = "tbchecklistid"
dim beditmode as boolean

sub mytreeview_findnode(byval strkey as string)
dim mynode as node

    for each mynode in me.tvwchecklist.nodes
...
dim icolumnid as integer
     ...
     if len(trim(validacamposobrigatorios(me))) > 0 then
        ...
        call savereg(me, stablename, stablefields, tvwchecklist, beditmode)
            
        beditmode = false
        
        call enabledisablecontrols(me, false)
        call sortmultiplecolumns(stablename, "categoria|ordem|item")
        icolumnid = application.worksheetfunction.match("#idchecklist", range(stablefields), 0)
        call carrega_treeview(stablename, stablefields, icolumnid, "categoria|item", tvwchecklist)
...
private sub tvwchecklist_nodeclick(byval node as mscomctllib.node)
dim obj as integer
